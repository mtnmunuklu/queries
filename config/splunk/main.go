...
func RunCommand(bridgePath, inputFilePath, configPath string) ([]byte, error) {
	cmd := exec.Command(bridgePath, "-filepath", inputFilePath, "-config", configPath, "-json", "-cs")
...
func ProcessFile(bridgePath, inputPath, outputPath, configPath, errorLogPath string) {
	err := filepath.Walk(inputPath, func(inputFilePath string, info os.FileInfo, err error) error {
		...
		if filepath.Ext(inputFilePath) == ".yml" {
			...
			jsonOutput, err := RunCommand(bridgePath, inputFilePath, configPath)
			if err != nil {
...
func main() {
	...
	bridgePath := flag.String("bridge", "", "Path to the bridge executable")
	...
	configPath := flag.String("config", "", "Path to the config file for the bridge command")

	// Parse flags
	flag.Parse()

	// Validate inputs
	if *bridgePath == "" || *inputDir == "" || *outputDir == "" || *configPath == "" {
		fmt.Println("Usage: -bridge <bridge-path> -input <input-dir> -output <output-dir> -config <config-path>")
		os.Exit(1)
	}

	// Process files and handle errors directly
	ProcessFile(*bridgePath, *inputDir, *outputDir, *configPath, "errors.log")
